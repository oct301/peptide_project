...
class en(tf.keras.Model):
    def __init__(self, num_layers, d_model, num_heads, dff,
                 input_vocab_size,
                 positional_encoding_input,
                 dropout_rate=0.1):
    ...
    def __call__(self, input, training, enc_padding_mask):
...
class de(tf.keras.Model):
    ...
    def __call__(self, target, enc_output, training, look_ahead_mask, dec_padding_mask):
...
class Transformer(tf.keras.Model):
    ...
    def en(self, input, training, enc_padding_mask):
        enc_output = self.encoder(input, training, enc_padding_mask)  # (batch_size, input_seq_len, d_model)
        return enc_output

    def de(self, target, enc_output, training, look_ahead_mask, dec_padding_mask):
        dec_output, attention_weights = self.decoder(
            target, enc_output, training, look_ahead_mask, dec_padding_mask)
